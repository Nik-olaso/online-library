...
3. Для файла `render_website.py`:
    ...
    Если вы не собираетесь запускать предыдущий файл `parse_tululu_category.py`, а планируете запустить рендер страниц только для тех книг, которые уже присутствует в репозитории, то вам следует создать файл `.env` собственноручно, со следующей информацией:

    - `BOOKS_FOLDER` – название общей папки для текста книг, их картинок и json-файла с параметрами книги.

    - `JSON_NAME` – название JSON-файла с информацией о книгах.

    Записать в таком формате:
    ```
    BOOKS_FOLDER = books_info(Любое желаемое название)
    JSON_NAME = books_params(Любое желаемое название)
    ```
    
    
    Теперь можно перейти к самому `render_website.py`
